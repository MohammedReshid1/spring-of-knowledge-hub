...
async def create_grade_level(
    ...
    # Only admin and superadmin can create grade levels
    if current_user.get("role") not in ["admin", "superadmin", "super_admin"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admin can create grade levels"
        )
...
async def list_grade_levels(
    ...
    if current_user.get("role") in ["superadmin", "super_admin"]:
        # Superadmin sees all grade levels
        pass
    else:
        # Regular users see only their branch's grade levels
        branch_id = current_user.get("branch_id")
        if not branch_id:
    ...
    async for g in coll.find(query):
