...
from ..db import get_classes_collection, validate_branch_id, validate_grade_level_id, validate_teacher_id
...
async def list_classes(
    ...
    async for c in coll.find():
        items.append(SchoolClass(id=str(c["_id"]), **{k: c.get(k) for k in c}))
    return items
...
async def get_class(
    ...
    return SchoolClass(id=class_id, **{k: c.get(k) for k in c})
