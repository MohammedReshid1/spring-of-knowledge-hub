...
from ..db import (
    ...
    get_student_collection, validate_student_id
)
...
async def create_exam_result(
    ...
    if current_user.get("role") not in ['super_admin', 'hq_admin', 'branch_admin', 'admin', 'teacher']:
        raise HTTPException(status_code=403, detail="Not authorized to create exam results")
...
async def get_exam_result(
    ...
    result = await results_coll.find_one({"_id": ObjectId(result_id)})
    if not result:
