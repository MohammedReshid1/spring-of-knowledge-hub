...
async def get_financial_summary(
    ...
    if branch_id:
        date_filter["branch_id"] = branch_id
    ...
    all_students = await student_collection.find({}).to_list(length=None)
...
async def get_attendance_summary(
    ...
    all_classes = await class_collection.find({}).to_list(length=None)
...
async def get_analytics_overview(
    ...
    overview = {
        ...
        "most_requested_report": "Student Report Cards",
        "average_generation_time": "2.3 seconds",
        "report_distribution": {
            "student_reports": 45,
            "class_reports": 23,
            "financial_reports": 12,
            "attendance_reports": 15,
            "academic_reports": 5
        },
        "format_preferences": {
            "PDF": 78,
        ...
        "scheduled_reports_active": 12,
        "templates_created": 24,
        "user_engagement": {
            "most_active_users": ["admin@school.com", "teacher1@school.com"],
...
async def get_performance_analytics(
    ...
    # Get exam results for the academic year
    exam_results = await exam_result_collection.find({
        "academic_year": academic_year
    }).to_list(length=None)
    
    # Get all students and grade levels
    all_students = await student_collection.find({}).to_list(length=None)
    all_grade_levels = await grade_level_collection.find({}).to_list(length=None)
    ...
    previous_results = await exam_result_collection.find({
