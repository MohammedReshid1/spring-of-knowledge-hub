...
async def create_incident(
    ...
    if incident_data.get("branch_id"):
        await validate_branch_id(incident_data["branch_id"])
...
async def get_disciplinary_statistics(
    ...
    # Basic counts
    total_incidents = await incidents_coll.count_documents({})
    open_incidents = await incidents_coll.count_documents({"status": "open"})
    resolved_incidents = await incidents_coll.count_documents({"status": "resolved"})
    
    # Behavior points
    positive_points_cursor = points_coll.aggregate([
        {"$match": {"point_type": "positive"}},
    ...
    # Other counts
    rewards_given = await rewards_coll.count_documents({})
    counseling_sessions_held = await sessions_coll.count_documents({"status": "completed"})
    behavior_contracts_active = await contracts_coll.count_documents({"status": "active"})
    parent_meetings_scheduled = await meetings_coll.count_documents({"status": {"$in": ["scheduled", "confirmed"]}})
    disciplinary_actions_pending = await actions_coll.count_documents({"status": "pending"})
